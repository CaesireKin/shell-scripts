...
if [ $mongo_express_admin_password ]; then
  ...
  docker run --name mongo-express --link $container_name:mongo -p 27018:8081 -e ME_CONFIG_OPTIONS_EDITORTHEME="ambiance" -e ME_CONFIG_MONGODB_SERVER="mongo" -e ME_CONFIG_BASICAUTH_USERNAME="admin" -e ME_CONFIG_BASICAUTH_PASSWORD="$mongo_express_admin_password" --restart always -d mongo-express;
fi