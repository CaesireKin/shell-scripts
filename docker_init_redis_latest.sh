...
if [ ! $redis_conf ]; then
  ...
  docker run --name $container_name -p 6379:6379 -v $docker_volumes/$container_name/conf/redis.conf:/usr/local/etc/redis/redis.conf -v $docker_volumes/$container_name/data:/data --memory=256M --memory-swap=256M --cpus=1 --restart always -d redis redis-server /usr/local/etc/redis/redis.conf --appendonly yes;
fi
